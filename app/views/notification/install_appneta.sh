...
install_debian() {
    ...
    case "$CODENAME" in
        hardy|intrepid|lucid|maverick|natty|oneiric|precise|quantal|raring|saucy|trusty|utopic|vivid|wily|etch|lenny|squeeze|wheezy|jessie) ;;
        lisa)
            CODENAME=oneiric ;;
        "")
    ...
    echo "deb http://$APT_SERVER/$ACCESS_KEY $CODENAME main" > appneta.list
...
install_redhat() {
    ...
    SUPPORTED_RELEASES="5-i386 5-x86_64 6-i386 6-x86_64 7-x86_64 2013-09-i386 2013-09-x86_64 2014-03-i386 2014-03-x86_64 2014-09-i386 2014-09-x86_64 2015-03-x86_64 2015-09-x86_64"
    FOUND_RELEASE=n
...
EOF
    ...
    if [ "$INSTALL_SEQUENCER" = YES ]; then
        if [ "${RELEASE}" != "5" ]; then
            ...
            echo_banner "The Sequencer is not available for Redhat 5. Skipping Sequencer installation."
        fi
    fi
...
print_usage_and_exit() {
    echo "Usage: $0 [--no-ca-cert] [--static] [--sequencer={yes|no}] ACCESS_KEY"
    ...
    echo "--sequencer: also install Appneta Sequencer for Network Monitoring (default: yes)"
...
# sequencer to be installed by default but for now, require explicit flag
INSTALL_SEQUENCER=NO
if $(grep -v -q '/$' /proc/1/cgroup); then
    # we are running in a container (e.g. Docker), so don't install sequencer by default
    INSTALL_SEQUENCER=NO
fi
...
for i in "$1" "$2" "$3" "$4"; do
    if [ -n "$i" ]; then
        case $i in
            ...
            ;;
            --sequencer=*)
            if [ -n "$(echo $i | grep -i =yes$)" ]; then
                INSTALL_SEQUENCER=YES
            elif [ -n "$(echo $i | grep -i =no$)" ]; then
                INSTALL_SEQUENCER=NO
            else
...
# do not install sequencer if already present
if [ "$INSTALL_SEQUENCER" = YES ]; then
    SEQ_FOUND=NO
    
    # check for Traceview Sequencer
    if debian_package_installed appneta-sequencer || redhat_package_installed appneta-sequencer; then
        SEQ_FOUND=YES
    fi

    # check for Pathview Sequencer
    if [ "$SEQ_FOUND" != "YES" ]; then
        # try to get install path from running process
        SEQ_PATH=$(readlink -f /proc/$(pgrep -f "netseq-linux" | head -n1)/exe | xargs dirname)
        if [ "$SEQ_PATH" = "/proc" ]; then
            # no netseq-linux process running, check default location
            SEQ_PATH="/opt/pathview"
        fi
        
        # check existence of nis.config
        if [ -e $SEQ_PATH/config/nis.config ]; then
            SEQ_FOUND=YES
        fi
    fi
    
    if [ "$SEQ_FOUND" = "YES" ]; then
        # sequencer found, disable installation
        INSTALL_SEQUENCER=NO
        echo_banner "There's already a Sequencer installed. Skipping installation of Traceview Sequencer."
    fi
    
    # in case the sequencer process is not running and it's not installed in the default path
    # then we're not able to find the install path
fi
...
if [ "$IS_RHEL_WORKSTATION" = "y" ]; then
    echo_banner "IMPORTANT NOTE: RHEL Workstation detected. Similar to RedHat policy on Workstation and Client, AppNeta software installed \
